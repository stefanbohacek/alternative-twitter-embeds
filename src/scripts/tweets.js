...
const ftfHelpers = {
  ...
  processTweets: () => {
    ...
    if (tweetIds.length) {
      if (CONFIG_USE_API) {
        ftfHelpers.fetchTweetData(tweetIds, (response) => {
          if (response && response.length) {
            ...
            for (const tweet of tweetsWithAttachment) {
              ...
              if (tweet.dataset.urlAttachment.indexOf("twitter.com/") > -1) {
                ftfHelpers.fetchTweetData(
                  [ftfHelpers.getTweetId(tweet.dataset.urlAttachment)],
                  (response) => {
                    if (response && response.length) {
                      response.forEach((data) => {
    ...
    if (data.media && data.media.length) {
      ...
      data.media.forEach((media, index) => {
        ...
        if (media.type === "animated_gif") {
          ...
          if (media.variants) {
            tweetText += `<video class="w-100 mt-0" controls loop><source src="${
